...
const MyThoughts = ({ myThoughts, getAllPost }) => {
  // useNavigation
  ...
  console.log(
    "MyThoughts.jsx - myThoughts prop:",
    myThoughts,
    "getAllPost prop:",
    getAllPost,
    "filteredPosts state:",
  ...
  // Use useEffect to update filteredPosts when getAllPost changes
  // getAllPost now contains merged posts from both JSON and backend sources
  useEffect(() => {
    // Use getAllPost which is already merged with both sources
    const postsToFilter = Array.isArray(getAllPost) ? getAllPost : [];
    setFilteredPosts(postsToFilter); //setting the post if available or empty []
  }, [getAllPost]);
  ...
  const handleInputChange = (e) => {
    ...
    // Filter the merged posts based on the title if search input is included
    const filteredItems = (Array.isArray(getAllPost) ? getAllPost : []).filter(
      ...
      <div className="w-full flex justify-between p-5 gap-4 flex-col lg:flex-row md:flex-row">
        ...
        <div className="w-[100%] relative h-[80vh] rounded-2xl shadow-[0px_4px_10px_rgba(0,0,0,0.25)] py-20 px-5 flex flex-col gap-5 overflow-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          {filteredPosts?.map((item, index) => {
            // Use filteredPosts here
            return (
              <div
                onClick={() => {
                  // Use compound ID: prefix with 'id_' for backend posts or 'idx_' for JSON posts
                  const postId = item._id ? `id_${item._id}` : `idx_${index}`;
                  navigation(`/articles/post/${postId}`);
                }}
                key={item._id || index} // Add a unique key, preferably item.id if available
